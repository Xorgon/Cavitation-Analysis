...
plot_3d_point_sets([[[p, q, 0]], centroids])
R_matrix = bem.get_R_matrix(centroids, normals, areas, dtype=np.float32)
R_inv = scipy.linalg.inv(R_matrix)

theta_js = []
R_b = bem.get_R_vector([p, q, 0], centroids, normals)
sigma = bem.calculate_sigma([p, q, 0], centroids, normals, areas, m_0, R_inv, R_b)

radii = np.linspace(0.1, 2, 16)
for radius in radii:
    print(f"Radius = {radius}")
    res_vel = get_avg_u(np.array([p, q, 0]), centroids, areas, sigma, radius, n=int(1e5))

...
from common.util.plotting_utils import plot_3d_point_sets, plot_2d_point_sets
import os
...
out_dir = "../model_outputs/asym_slots/"

if not os.path.exists(out_dir) and save_to_file:
    os.makedirs(out_dir)

# centroids, normals, areas = gen.gen_slot(n=n, h=h, w=w, length=50, depth=50)
centroids, normals, areas = gen.gen_asymmetric_varied_slot(n=n, H=H, W=W, length=100, depth=50, w_thresh=w_thresh,
                                                           asym_angle=asym_angle, density_ratio=density_ratio)
print("Requested n = {0}, using n = {1}.".format(n, len(centroids)))

plot_2d_point_sets([centroids])
...
for Y in Ys:
    ...
    print(f"Difference in peaks = {np.max(thetas) + np.min(thetas)}")

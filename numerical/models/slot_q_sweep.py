...
for i, q in enumerate(qs):
    ...
    label_frac = "\\frac{q}{w}"
    if i == 0:
        ax.plot(p_bar_to_plot, theta_j_to_plot, label=f"${label_frac} = {q / w:.2f}$")
        norm_ax.plot(norm_p_bar_to_plot, norm_theta_j_to_plot, label=f"${label_frac} = {q / w:.2f}$")
    else:
        ax.plot(p_bar_to_plot, theta_j_to_plot, label=f"${label_frac} = {q / w:.2f}$", linestyle="--",
                dashes=(i, 2 * i))
        norm_ax.plot(norm_p_bar_to_plot, norm_theta_j_to_plot, label=f"${label_frac} = {q / w:.2f}$", linestyle="--",
                     dashes=(i, 2 * i))
...
ax.legend(bbox_to_anchor=(0, -0.34, 2.3, .05), loc=10, ncol=len(qs), mode="expand",
          borderaxespad=0,

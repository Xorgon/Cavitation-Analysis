...
def get_2d_vel(x_b, y_b, w=1.0, n=25, Q=1.0):
    """
    Calculate the velocity for a bubble position with the given geometric parameters.
    :param x_b: Bubble x position, measured from the left (closed) end.
    :param y_b: Bubble y position, measured from the horizontal plane of symmetry.
    ...
    for i in [-1, 1]:
        for j in range(-n, n + 1):
            ...
            sink_coords.append([s_x, s_y])

    # Sum the contribution from all sinks
    vel = [0.0, 0.0, 0.0]
    for (s_x, s_y) in sink_coords:
        R = math.sqrt((x_b - s_x) ** 2 + (y_b - s_y) ** 2)
        vel[0] += - Q * (x_b - s_x) / (2 * math.pi * R ** 2)
        vel[1] += - Q * (y_b - s_y) / (2 * math.pi * R ** 2)
    return vel


# Geometry parameters
width = 400e-6
heights = np.linspace(25e-6, 250e-6, 4)
...
for height in heights:
    ...
    for y in ys:
        ...
        vel = get_vel(x, y, 0, width, height, n=50, Q=1e-6)
        theta = math.atan2(vel[1], -vel[0])
        thetas.append(theta)

    plt.plot(ys * 1e6, thetas, label="height$ = {0:.0f}\\mu$m".format(height * 1e6))

# Try 2D
thetas = []
for y in ys:
    vel = get_2d_vel(x, y, width, n=50, Q=1e-6)
    theta = math.atan2(vel[1], -vel[0])
    thetas.append(theta)
plt.plot(ys * 1e6, thetas, label="2D sinks")

...
min_target_ani = 3.7e-2
max_target_ani = 3.8e-2
...
for j, (label, max_ecc, alpha, marker, m_size, colour, dirs) in enumerate(sets):
    ...
    for i, dir_path in enumerate(dirs):  # type: int, str
        ...
        try:
            ...
            # Find anisotropies and stuff
            density = 997
            P_vapour = 2.3388e3
            P_inf = 100e3
            print(dir_path)
            [print(f"    {r.idx}:{r.repeat_number}"
                   f" - zeta = {r.get_scalar_anisotropy()}"
                   f" - {np.rad2deg(r.get_jet_angle())} deg"
                   f" - {1e6 * 0.915 * (r.get_max_radius(params.mm_per_px) / 1000) * (density / (P_inf - P_vapour)) ** 0.5} us"
                   f" - {np.sqrt(r.max_bubble_area / np.pi)} px")
             for r in readings if r_filter(r) and min_target_ani <= r.get_scalar_anisotropy() <= max_target_ani]
...
legend = disp_ax.legend(loc='lower right', frameon=False, fontsize='xx-small', markerfirst=False)
...
r_ax.legend(loc='lower right', frameon=False, fontsize='xx-small')

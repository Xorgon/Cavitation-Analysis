...
xs = np.linspace(2 * offset, L * np.sqrt(3) / 2 - offset, 32)
...
for i, j in itertools.product(range(len(xs)), range(len(ys))):
    print(i, j)
    if not (- (xs[i] - 2 * offset) / np.sqrt(3) <= ys[j] <= (xs[i] - 2 * offset) / np.sqrt(3)):
...
if not os.path.exists("../../model_outputs/triangle_anisotropy_data"):
    os.makedirs("../../model_outputs/triangle_anisotropy_data")
lists_to_csv(f'../../model_outputs/triangle_anisotropy_data/', f'R{1000 * R_init:.1f}_L{1000 * L:.0f}_n{n}_i{img_ranges}',
...
plt.plot([0, L * np.sqrt(3) / 2, L * np.sqrt(3) / 2, 0],
         [0, L / 2, - L / 2, 0], 'k')

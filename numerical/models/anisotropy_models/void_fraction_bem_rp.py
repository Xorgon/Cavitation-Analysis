...
vfs = np.linspace(0, 0.99, 10)
anisotropies = []
for vf in vfs:
    print(f"Void fraction = {vf:.2f}")
    centroids, normals, areas = gen.gen_varied_simple_plane(0.05, 10000, 2, 2000)
    areas *= (1 - vf)
    R_matrix = bem.get_R_matrix(centroids, normals, areas)
    R_inv = np.linalg.inv(R_matrix)

    bubble_pos = np.array([0, standoff * R_init, 0])
...
plt.scatter(vfs, anisotropies, label="BEM")
plt.xlabel("$\\phi$")
plt.ylabel("$\\zeta$")
plt.legend()

plt.figure()
plt.scatter(vfs, 2.5 * np.array(np.abs(anisotropies)) ** (3 / 5))
plt.xlabel("$\\phi$")
plt.ylabel("Supponen normalised bubble displacement")
plt.show()

...
for reading_y in reading_ys:
    ...
    lin_fit_coeffs = np.polyfit(areas, theta_js_normalized, 1)
    ...
    plt.scatter(areas, theta_js_normalized, c=p_bars)
    plt.plot(fit_areas, fit_theta_js, 'r')
    ...
    plt.xlabel("$\\bar{p}$")
    plt.ylabel("$\\theta_j$ deviation (rads)")
    plt.scatter(p_bars, theta_j_deviations)
    plt.show()

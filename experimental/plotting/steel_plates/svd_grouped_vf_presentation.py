...
for i, geom_dir in enumerate(geoms):
    ...
    for root, _, files in os.walk(root_dir + geom_dir):
        ...
        if "params.py" in files:
            ...
            if W / np.mean([r.get_max_radius(params.mm_per_px) for r in readings if filter_by(r)]) > 1.5:
                print(geom_dir, W / np.mean([r.get_max_radius(params.mm_per_px) for r in readings if filter_by(r)]))
                break
...
for k in range(len(vf_standoffs.keys()) + 1):
    fig, (disp_ax, ratio_ax) = plt.subplots(1, 2, figsize=(6, 3))
    for i, vf in enumerate(sorted(vf_standoffs.keys())[:k]):
        ...
        if fit_for_disp_cuts:
            ...
            # if i == 0 or i == round(len(vf_standoffs.keys()) / 2 - 1) or i == len(vf_standoffs.keys()) - 1:
    ...
    ratio_ax.set_xlim((1, 7))
    disp_ax.set_ylim((0.025, 1.4))
    ratio_ax.set_ylim((0.12, 0.8))
    ...
    plt.tight_layout()
    plt.savefig(f"C:/Users/eda1g15/OneDrive - University of Southampton/Research/Miscellaneous/45 minute presentation/"
                f"figures/svd_vf_{k}.png")

    if k == len(vf_standoffs.keys()):
        for h, stdoff in enumerate(standoff_cuts):
            disp_ax.axvline(stdoff, color="gray", linestyle="dashed")
            ratio_ax.axvline(stdoff, color="gray", linestyle="dashed")
            plt.savefig(
                f"C:/Users/eda1g15/OneDrive - University of Southampton/Research/Miscellaneous/45 minute presentation/"
                f"figures/svd_vf_{k}_c{h}.png")

fig, (disp_cuts_ax, ratio_cuts_ax) = plt.subplots(1, 2, figsize=(6, 3))
for j, stdoff in enumerate(standoff_cuts):

...
opt_res = minimize(opt_polyfit, np.zeros((len(phis), 1)))
optimal_offs = opt_res.x

all_stdoff_ratio_phis = []
all_stdoff_ratios = []
...
for n in range(0, len(optimal_offs)):
    adjusted_standoffs = []
    all_disps = []
    all_radius_ratios = []
    for stdoff, disp, ratio in zip(standoff_lists[n], displacement_lists[n], ratio_lists[n]):
        adjusted_standoffs.append(stdoff + optimal_offs[n])
        combined_adjusted_standoffs.append(stdoff + optimal_offs[n])
        all_disps.append(disp)
        combined_disps.append(disp)
        all_radius_ratios.append(ratio)
        all_stdoff_ratio_phis.append(phis[n])
        all_stdoff_ratios.append(1 + optimal_offs[n] / stdoff)
...
plt.figure()
plt.scatter(phis, optimal_offs)
plt.xlabel("$\\phi$")
plt.ylabel("$\\delta = \\gamma$ offset")
plt.tight_layout()

plt.figure()
plt.scatter(all_stdoff_ratio_phis, all_stdoff_ratios, label="All $\\gamma_e / \\gamma$", alpha=0.1, s=5)
plt.scatter(stdoff_ratio_phis, mean_stdoff_ratios, label="Mean $\\gamma_e / \\gamma$")
plt.plot(np.linspace(plt.xlim()[0], plt.xlim()[1]), np.sqrt(1 / (1 - np.linspace(plt.xlim()[0], plt.xlim()[1]))),

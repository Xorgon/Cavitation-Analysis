...
corner_dirs = ["E:/Data/Ivo/Restructured Data/60 degree corner/"]
...
for i, dir_path in enumerate(corner_dirs):
    try:
        ...
        x_offset = params.corner[0]
        y_offset = params.corner[1]
        ...
        standoffs = [np.abs((np.arctan2(r.get_bubble_pos_mm(params.mm_per_px)[1] - y_offset,
                                 r.get_bubble_pos_mm(params.mm_per_px)[0] - x_offset) - np.pi / 2))
                     for r in readings if r.ecc_at_max < max_ecc]

        disp_ax.scatter(standoffs,
                        [np.linalg.norm(r.get_normalised_displacement()) for r in readings if
                         r.ecc_at_max < max_ecc],
                        marker=".", color=f'C{i}', alpha=0.5)

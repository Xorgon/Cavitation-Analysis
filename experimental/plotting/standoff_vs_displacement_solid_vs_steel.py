...
groups = [
    ...
    ["3 mm acrylic 50 mm solid plate",
...
for label, dirs, phi in groups:
    ...
    for i, (dir_path, _, colour, marker) in enumerate(dirs):
        ...
        for j, reading in enumerate(readings):
            ...
            if y >= 0 and reading.ecc_at_max < 0.3:
                ys.append(y)
...
size_ratio_fig.gca().plot([1.6, 2.0, 3.0], np.power([0.8077, 0.6400, 0.4345], 1 / 3),
                          marker="o", label="Wang (2016) - Numerical", c="red")
size_ratio_fig.gca().plot([2.38, 1.5, 0.86], [0.636, 0.686, 0.712], marker="o",
                          label="Yin et al. (2021) - Numerical/Experimental", c="#00ff00")
size_ratio_fig.gca().plot([1.37, 2.15], [0.52, 0.41], marker="o",
                          label="Zhang (2021) - Numerical 'with phase transition'", c="purple")
...
for i, gamma in enumerate(cross_section_gammas):
    ...
    cross_section_fig.gca().text(np.mean(cross_section_phis[i]), np.mean(cross_section_disps[i]),

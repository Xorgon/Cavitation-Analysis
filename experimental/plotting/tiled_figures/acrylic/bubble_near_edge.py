...
repeat = 1
frame_idxs = [27, 38, 41, 48, 63]

tile_tl_corner = (141, 53)
tile_br_corner = (273, 207)
img_scale = 8
fig_scale = 1

f_dir = "C:/Users/eda1g15/OneDrive - University of Southampton/Research/Porous Materials/Data/~25VF/On hole/movie_C001H001S0001/"
mm_per_px = 0.0250
...
for k, idx in enumerate(frame_idxs):
    ...
    # FILL
    ax.add_patch(Polygon([(img_scale * tile_tl_corner[0], top_y),
                          (1396, top_y),
                          (1396, bot_y),
                          (img_scale * tile_tl_corner[0], bot_y)],
    ...
    ax.add_patch(Polygon([(img_scale * tile_tl_corner[0], top_y),
                          ...
                          (1396, bot_y),
                          (img_scale * tile_tl_corner[0], bot_y)],
                         edgecolor="white", lw=1, fill=False))


    # FILL
    ax.add_patch(Polygon([(img_scale * tile_br_corner[0], top_y),
                          (1880, top_y),
                          (1880, bot_y),
                          (img_scale * tile_br_corner[0], bot_y)],
                         facecolor="white", hatch="////"))
    # BORDER
    ax.add_patch(Polygon([(img_scale * tile_br_corner[0], top_y),
                          (1880, top_y),
    ...
    ax.plot([1396, 1880], [top_y, top_y], "w--")
    ax.plot([1396, 1880], [bot_y, bot_y], "w--")

    if k == 1 and include_scale:

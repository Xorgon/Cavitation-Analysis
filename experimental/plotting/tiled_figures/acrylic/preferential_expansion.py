...
for k, idx in enumerate(frame_idxs):
    ...
    top_y = 1230
    bot_y = img_scale * tile_br_corner[1]
    solid_left = 1260
    solid_right = 1822

    # FILL
    ax.add_patch(Polygon([(solid_left, top_y),
                          (solid_right, top_y),
                          (solid_right, bot_y),
                          (solid_left, bot_y)],
                         facecolor="#ffffff", hatch="////"))
    # BORDER
    ax.add_patch(Polygon([(solid_left, top_y),
                          (solid_right, top_y),
                          (solid_right, bot_y),
                          (solid_left, bot_y)],
                         edgecolor="#ffffff", lw=1, fill=False))

    ax.plot([img_scale * tile_tl_corner[0], solid_left], [top_y, top_y], "w--")
    ax.plot([img_scale * tile_tl_corner[0], solid_left], [bot_y, bot_y], "w--")

    ax.plot([solid_right, img_scale * tile_br_corner[0]], [top_y, top_y], "w--")
    ax.plot([solid_right, img_scale * tile_br_corner[0]], [bot_y, bot_y], "w--")

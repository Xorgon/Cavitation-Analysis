...
if not os.path.exists("slot_anisotropy_data"):
    os.makedirs("slot_anisotropy_data")
...
centroids_file = open(f"slot_anisotropy_data/centroids_n{n}_W{W * 1000:.2f}_H{H * 1000:.2f}"
                      f"_drat{density_ratio}_wthresh{w_thresh:.2f}_len{length * 1000:.1f}.csv", 'w')
for c in centroids:
    centroids_file.write(f"{c[0]},{c[1]},{c[2]}\n")
centroids_file.close()

normals_file = open(f"slot_anisotropy_data/normals_n{n}_W{W * 1000:.2f}_H{H * 1000:.2f}"
                    f"_drat{density_ratio}_wthresh{w_thresh:.2f}_len{length * 1000:.1f}.csv", 'w')
for normal in normals:
    normals_file.write(f"{normal[0]},{normal[1]},{normal[2]}\n")
normals_file.close()

output_path = f"slot_anisotropy_data/anisotropy_sweep_n{n}_W{W * 1000:.2f}_H{H * 1000:.2f}" \

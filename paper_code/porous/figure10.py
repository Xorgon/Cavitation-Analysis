...
from util.drawing_utils import draw_plate
...
for i, geom_dir in enumerate([geom_dir for geom_dir in geoms if geom_filter(geom_dir)]):
    ...
    for root, _, files in os.walk(root_dir + geom_dir):
        if "params.py" in files:
            ...
            small_as.append(A / (np.pi * mean_radius ** 2))
...
iax = disp_ax.inset_axes([0.69, 0.69, 0.275, 0.275])
draw_plate(iax, vfs[0], ws[0], label="Non-porous", color="C0", linewidth=3)
...
iax = r_ax.inset_axes([0.69, 0.69, 0.275, 0.275])
...
disp_ax.set_xticks([2, 3, 4, 5, 6])
r_ax.set_xticks([2, 3, 4, 5, 6])
